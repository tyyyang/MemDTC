...
def rnn(cell, inputs, initial_state, scope=None):
    ...
    if len(inputs) >= config.summary_display_step:
        for i in range(config.summary_display_step):
            state = states[i]
            weights_summay(state.access_state_pos.memory, 'memory_slot/{}'.format(i))
            weights_summay(state.access_state_pos.read_weight, 'read_weight/{}'.format(i))
            weights_summay(state.access_state_pos.write_weight, 'write_weight/{}'.format(i))
            weights_summay(state.access_state_pos.usage, 'usage/{}'.format(i))
            weights_summay(state.access_state_neg.memory, 'memory_slot_neg/{}'.format(i))
