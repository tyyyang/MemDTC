...
def cls_branch(input, is_train, name):

    input_shape = input.get_shape().as_list()
    if len(input_shape) > 4:
        input = tf.reshape(input, [-1] + input_shape[2:])

    output = bn_relu_conv2d(is_train, input, 1024, config.slot_size[0:2], [1, 1], 'valid', name=name)

    if len(input_shape) > 4:
        output_shape = output.get_shape().as_list()
        output = tf.reshape(output, input_shape[0:2]+output_shape[1:])
